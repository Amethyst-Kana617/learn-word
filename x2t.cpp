...
map<wstring, wstring> loadWordSet(const string& filename) {
    ...
    wifstream inFile(filename);      // 打开文件，准备进行读取
    ...
    wstring line; // 声明一个宽字符串来存储文件的每一行
    while (getline(inFile, line)) { // 逐行读取文件内容
        size_t delimiterPos = line.find(L'|'); // 查找'|'字符的位置
        if (delimiterPos != wstring::npos) { // 如果找到了'|'字符
            wstring english = line.substr(0, delimiterPos); // 提取'|'字符前的部分作为英文单词
            wstring chinese = line.substr(delimiterPos + 1); // 提取'|'字符后的部分作为中文意思
            wordSet[english] = chinese; // 将英文单词和中文意思存储到映射中
        }
    }
