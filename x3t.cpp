...
int main() {
    ...
    // 将 map 中的元素转移到 vector 中
    vector<pair<string, string>> wordVec(wordSet.begin(), wordSet.end());

    // 使用随机数引擎来打乱 vector 中的顺序
    unsigned seed = std::chrono::system_clock::now().time_since_epoch().count(); // 获取当前时间作为随机种子
    shuffle(wordVec.begin(), wordVec.end(), std::default_random_engine(seed));

    int correctCount = 0; // 正确的单词数量计数器
    int remainingWords = wordVec.size(); // 剩余单词数量计数器
